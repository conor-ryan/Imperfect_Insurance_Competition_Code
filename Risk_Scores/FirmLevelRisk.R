...
# firm_level_risk_optim = function(R,df){
#   df$R_wtd[df$Firm!="OTHER"] = R
#   df$R_wtd[df$Firm=="OTHER"] = df$A_wtd[df$Firm=="OTHER"]
#   
#   df$A_sum = ave(df$A_wtd,FUN=sum)
#   df$R_sum = ave(df$R_wtd,FUN=sum)
#   df$transfer_pred=with(df,-mkt_Lives*avg_prem*(R_wtd/R_sum - A_wtd/A_sum))
#   
#   error = with(df[df$Firm!="OTHER",],payments_adj-transfer_pred)
#   return(sum(error^2))
# }

res = nleqslv(x=R,fn=firm_level_risk)
