    ...
    while (cnt<max_itr)
        ...
        while ((f_test>fval*mistake_thresh) | isnan(f_test)) & (trial_max==0)
            ...
            if (step_size>x_tol)
                ...
                p_test, f_test = gradient_ascent_BB(p0,par_est,m,costdf,W,max_itr=5,strict=true)
            end
        end
...
function estimate_GMM(p0::Vector{Float64},p_est::parDict{Float64},
    ...
    p_stg1, fval = gradient_ascent_BB(p0,p_est,d,c,W,max_itr=20)
    # Then run newtons method until better convergence
    println("Newtons Method")
    p_est, fval = newton_raphson_GMM(p_stg1,p_est,d,c,W,grad_tol = 1e-8,strict=true)
