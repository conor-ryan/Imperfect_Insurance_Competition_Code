function costMoments!(hess::Array{T,3},grad::Matrix{T},c::MC_Data,d::InsuranceLogit,p::parMC{T}) where T
    ...
    d_amom = Vector{T}(undef,length(c.ageMoments))
    ...
    for k in vcat(c._baseIndex,c._feIndex,c._riskIndex)
        ...
        for l in l_index
            ...
            for (m,m_idx) in c._avgMomentDict
                ...
                hess[k,l,m] = d2c_avg/c_avg
                hess[l,k,m] = d2c_avg/c_avg
            end
            ...
            for (m,m_idx) in c._ageMomentDict
                if m==1
                    ...
                    hess[k,l,M1 + m-1] = d2c_avg/refval - dlrefval[1]*dkc_avg/(refval)^2 - d2refval[1]*c_avg/(refval)^2 - dkrefval[1]*dlc_avg/(refval)^2 + dkrefval[1]*dlrefval[1]*c_avg/(refval)^3
                    hess[l,k,M1 + m-1] = d2c_avg/refval - dlrefval[1]*dkc_avg/(refval)^2 - d2refval[1]*c_avg/(refval)^2 - dkrefval[1]*dlc_avg/(refval)^2 + dkrefval[1]*dlrefval[1]*c_avg/(refval)^3
                end
            end
            ...
            hess[k,l,c.mom_length] = d2HCC_avg/non_avg - dlnon_avg*dkHCC_avg/non_avg^2 - dknon_avg*dlHCC_avg/non_avg^2 - d2non_avg*HCC_avg/non_avg^2 + dknon_avg*dlnon_avg*HCC_avg/non_avg^3
            hess[l,k,c.mom_length] = d2HCC_avg/non_avg - dlnon_avg*dkHCC_avg/non_avg^2 - dknon_avg*dlHCC_avg/non_avg^2 - d2non_avg*HCC_avg/non_avg^2 + dknon_avg*dlnon_avg*HCC_avg/non_avg^3
        end
    end
