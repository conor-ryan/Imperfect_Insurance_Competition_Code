...
function individual_values!(d::InsuranceLogit)
    ...
    for app in eachperson(d.data)
        ...
        for n in 1:N, (k,i) in enumerate(idxitr)
            d.μ[i,n] = chars[k,n] + demos
        end
    end
    return Void
end
...
function individual_shares_RC!(d::InsuranceLogit)
    perLookup = d.data._personDict
    # Normalize to outside option
    for idx_i in values(perLookup)
        K = length(idx_i)
        N = size(d.draws,2)
        util = Matrix{Float64}(K,N)
        s_hat = Matrix{Float64}(K,N)
        for n in 1:N
            expsum = 0.0
            for (i,l) in enumerate(idx_i)
                a = exp(d.μ[l,n] + d.δ[l])
                util[i,n] = a
                expsum += a
            end
...
function log_likelihood(d::InsuranceLogit)
    ll = 0.0
    Y = transpose(choice(d.data))
    urate = transpose(unins(d.data))
    ...
    for idx_i in values(perLookup)
        ...
        s_mean = mean(s_hat,2)
        d.s_hat[idx_i] = s_mean
        s_insured = sum(s_mean)
        for (i,l) in enumerate(idx_i)
            # ll+=Y[l]*log(d.s_hat[l]/s_insured)
            # ll+=Y[l]*(1-urate[l])*log(s_insured)
            ll+=Y[l]*(log(d.s_hat[l])-urate[l]*log(s_insured))
        end
    end
    return ll
end
...
# Evaluate one iteration, given intitial parameters
