...
function newton_raphson_ll(d,p0,W;grad_tol=1e-8,f_tol=1e-8,x_tol=1e-10,
    ...
    while (grad_size>grad_tol) & (cnt<max_itr) & (max_trial_cnt<20)
        ...
        if (cnt==1) | (fval>=f_min)
            if (abs(fval-f_min)<f_tol) & (real_hessian==1)
                f_tol_cnt += 1
            end
        ...
        while ((f_test<fval*mistake_thresh) | isnan(f_test)) & (trial_max==0)
            ...
            step_size = maximum(abs.(update))
            if ((real_hessian==0) & (step_size>1e-4)) | ((real_hessian==1) & (step_size>1e-15))
                p_test = p_vec .+ update
                f_test = log_likelihood_penalty(d,p_test,W)
                p_test_disp = p_test[disp_vec]
            ...
            #     hess_steps=0
            # end
            # if (step_size<x_tol) & (real_hessian==1)
            #     x_tol_cnt +=1
            # end
        end
        ...
        #     f_tol_flag = 0
        # end
        if (step_size<x_tol) & (f_test>=fval)
            x_tol_cnt +=1
        else
            x_tol_cnt = 0
        end
