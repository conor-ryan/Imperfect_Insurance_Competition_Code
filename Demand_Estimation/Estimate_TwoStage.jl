function two_stage_est(d,p0,W;grad_tol=1e-8,f_tol=1e-8,x_tol=1e-10,
    ...
    constraint = 10.0
    constrained = 0
    bound_ind = Int.(1:length(par_ind))
    ...
    p_vec, f_test,ga_conv = ga_twostage(d,p_vec,W,par_ind,max_itr=15,strict=false,Grad_Skip_Steps=2)
    ...
    while  (cnt<max_itr) & (max_trial_cnt<20)
        ...
        while ((f_test<fval*mistake_thresh) | isnan(f_test)) & (trial_max==0)
            ...
            elseif real_hessian==1
                ...
                p_test = copy(p_vec)
                p_test[FE_ind] = p_vec[FE_ind] .+ rand(length(FE_ind)).*0.05
                hess_steps=0
                break
            else
