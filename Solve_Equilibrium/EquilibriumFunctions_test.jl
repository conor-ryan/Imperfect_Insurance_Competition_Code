...
function gsum_by_person(x::Vector{Float64},e::EqData)
    out = zeros(length(x))
    for (n,i) in enumerate(e.data._personIDs)
        idx_i = e.data._personDict[i]
        sumvar = 0.0
...
function gsum_by_person(x::Vector{Float64},e::EqData,weights::Vector{Float64})
    out = zeros(length(x))
    for (n,i) in enumerate(e.data._personIDs)
        idx_i = e.data._personDict[i]
        sumvar = 0.0
        for q in idx_i
            sumvar+= x[q]*weights[q]
        end
        out[idx_i] = sumvar
    end
    return out
...
function eval_FOC(e::EqData)
    ...
    All_Lives = sum(e.s_pred.*e.data[:PERWT])
    ...
    #### Market Pooled Cost ####
    PC = sum(e.s_pred.*e.data[:PERWT].*Cost_long)/sum(e.s_pred.*e.data[:PERWT])
    PC = PC*sum(e.data[:PERWT])/sum(ageRate_long.*e.data[:PERWT])
    #PC = sum(S_m.*S_j.*e[:C])/sum(S_m.*S_j)
    Ec = sum(Cost_long.*weights)/sum(ageRate_long.*weights)
    τ = PC - Ec
    ...
    for (n,j) in enumerate(e.prods)
        ...
        deriv_byperson = gsum_by_person(deriv_long_byperson,e)
        deriv_byperson = deriv_byperson[e.data._per_2_prod_map]
        ...
        #
        dMCost[:,n] = sum_by_prod(deriv_byperson,Cost_long,e,weights)

        dτ = sum(deriv_long.*Cost_long.*e.data[:PERWT])/sum(e.s_pred.*e.data[:PERWT]) -
                        sum(deriv_long.*e.data[:PERWT])/sum(e.s_pred.*e.data[:PERWT])*PC
    ...
    for (m,m_idx) in e.mkt_index
        ...
        P_RA[m_idx] = inv(m_dsdp_rev)*(-S.*A + sum(m_dsdp_rev,1)'.*PC - sum(all_dsdp_rev,1)'.*PC.*Sf + sum(all_dCost,1)'.*Sf)

        P_RAτ[m_idx] = inv(m_dsdp_rev_dτ)*(-S.*A) + Ec
