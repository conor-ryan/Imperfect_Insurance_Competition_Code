...
output_data = Matrix{Float64}(undef,Int(M*(N+1)),23)
...
any_long = anyHCC(m.data)
demData = demoRaw(m.data)

state_index = Vector{String}(undef,Int(M*(N+1)))
state_long = df[:ST]
ind = [0]
for i in 1:M
    if i%1000==0
    ...
    Z = demData[:,i]
    α = (par_est_dem.β_0 + par_est_dem.β*Z)[1]


    dens_risk = anyRisk*density_long[i]/N
    wgt_risk = anyRisk*wgts_long[i]/N

    dens_nonrisk = (1-anyRisk)*density_long[i]
    wgt_nonrisk = (1-anyRisk)*wgts_long[i]
    ...
    for n in 1:N
        ...
        per = per_raw*1e6*(N+1) + n
        R = risk_draw + ageHCC
        C = C_nonrisk*par_est_mc.risks[n,r_indS]
        C_nonAV = C/C_AV
        # α = par_est_dem.α[n,i]
        # μ_ij_nonprice = par_est_dem.μ_ij_nonprice[n,i]
        μ_ij = par_est_dem.μ_ij[n,i]*δ
        μ_ij_nonprice = μ_ij/(exp(α*p_base))

        row = vcat([per,prd,catas,dens_risk,wgt_risk],oth,[α,μ_ij_nonprice,μ_ij,R,risk_HCC,C,C_nonAV,s_pred,p_base])
        # row = vcat([per,prd,catas,dens_risk,wgt_risk],oth,[μ_ij,R,risk_HCC,C,C_nonAV,s_pred,p_base])
    ...
    # α = par_est_dem.α[n,i]
    # μ_ij_nonprice = par_est_dem.μ_ij_nonprice[n,i]
    μ_ij = par_est_dem.μ_ij_nonRisk[i]*δ
    μ_ij_nonprice = μ_ij/(exp(α*p_base))

    row = vcat([per,prd,catas,dens_nonrisk,wgt_nonrisk],oth,[α,μ_ij_nonprice,μ_ij,R,risk_HCC,C,C_nonAV,s_pred,p_base])
    # row = vcat([per,prd,catas,dens_nonrisk,wgt_nonrisk],oth,[μ_ij,R,risk_HCC,C,C_nonAV,s_pred,p_base])
    output_data[ind[1],:] = row
    state_index[ind[1]] = state_long[i]
...
# header = Symbol.(["Person","Product","Catastrophic","mkt_density","PERWT",
# "AGE","ageRate","ageRate_avg","AV","Gamma_j","Mandate","subsidy","IncomeCont","MEMBERS",
# "util","R","R_HCC","C","C_nonAV","s_base","p_base"])
for st in states
    println(st)
